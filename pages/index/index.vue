<template>
	<view>
		<view class="search">
			<uni-search-bar radius="5" placeholder="请输入你要搜索的内容" clearButton="auto" cancelButton="none" @confirm="search" />
		</view>
		<view class="loop-img">
		...
		<view>
			...
			<view class="news-list" style="padding: 20rpx;">
				<uni-list>
					<uni-list-item title="" note="" v-for="(item,index) in news.newsList_jinri" :key="index" :title="item.title" :note="item.createTime.slice(0,10)+' '+'评论数:'+item.commentNum">
						<image :src="'http://124.93.196.45:10001'+item.cover" slot="header" style="width: 200rpx; height: 150rpx; border-radius: 5px; margin-right: 10px;"></image>
					</uni-list-item>
				</uni-list>
			</view>
		</view>
